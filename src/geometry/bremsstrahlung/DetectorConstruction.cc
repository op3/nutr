...
G4VPhysicalVolume *DetectorConstruction::Construct()
{
    ...
    const auto world_x = 40. * mm;
    const auto world_y = 40. * mm;
    const auto world_z = 1.2 * m;
    ...
    RadiatorTarget.Construct(G4ThreeVector(0, 0, -1120. * mm + 0.5 * world_z), "Radiator1", "Au", 2.5 * mm, "Al", 0.); // Position guesstimated
    ...
    const auto exit_window_distance = -1150. * mm;
    ...
    auto target = new G4Tubs(
        "target_solid", 0. * mm, target_radius, 1. * mm, 0. * deg, 360. * deg);
    auto target_logical = new G4LogicalVolume(
        target, nist_manager->FindOrBuildMaterial("G4_AIR"), "target_logical");
    target_logical->SetVisAttributes(G4Color::Red());
    new G4PVPlacement(new G4RotationMatrix(), G4ThreeVector(0, 0, .5 * world_z), target_logical, "target", world_logical.get(), false, 0);
